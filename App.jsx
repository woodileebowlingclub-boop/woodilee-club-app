...
function MondayPointsAdmin({ members = [] }) {
  ...
  useEffect(() => {
    ...
    const existingWeek = Object.values(points).some(
      (memberPoints) => memberPoints?.[selectedDate] !== undefined
    );
    ...
    members.forEach((member) => {
      ...
      if (updated[memberName][selectedDate] !== undefined) return;

      if (previousDate && updated[memberName][previousDate] !== undefined) {
        updated[memberName][selectedDate] = updated[memberName][previousDate];
      } else {
        updated[memberName][selectedDate] = 0;
      }
    });

    const changed = JSON.stringify(updated) !== JSON.stringify(points);
    if (changed) {
      savePoints(updated);
    }
  }, [selectedDate, members, points]);
  ...
  const exportToExcel = () => {
    ...
    const rows = sortedMembers.map((member) => {
      const memberPoints = points[member.name] || {};
      const row = { Member: member.name };
