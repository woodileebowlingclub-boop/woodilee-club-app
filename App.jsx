...
const styles = {
  ...
  prizeRow1: { background: "#fff7cc" },
  prizeRow2: { background: "#edf2f7" },
  prizeRow3: { background: "#fbe4d5" },
};
...
function MondayPointsAdmin({ members = [] }) {
  ...
  const displayedPlayers = useMemo(() => {
    const playerSet = new Set();
    allPreviousPlayers.forEach((name) => playerSet.add(name));
    ...
    return Array.from(playerSet).sort((a, b) => a.localeCompare(b, "en-GB"));
  }, [allPreviousPlayers, points, selectedDate]);
  ...
  useEffect(() => {
    ...
    allPreviousPlayers.forEach((memberName) => {
        ...
        <tbody>
          {displayedPlayers.length === 0 ? (
            ...
            displayedPlayers.map((memberName) => (
              <tr key={memberName}>
                <td style={{ ...styles.adminTd, ...styles.stickyColBody }}>{memberName}</td>
...
export default function App() {
  ...
  const savePost = async () => {
    if (!postTitle || !postMessage || !postDate) return setMessage("Enter post title, message and date.");
    ...
    <div style={styles.page}>
      <div style={styles.wrap}>
        ...
        <div style={styles.tabs}>
          ...
          <button style={styles.tab(tab === "leaderboard")} onClick={() => setTab("leaderboard")}>Monday Points Leaderboard</button>
          ...
          <>
            ...
            <div style={styles.homeGrid}>
              <div style={styles.homeMiniCard}>
                ...
                <button style={styles.homeActionBtn} onClick={() => setTab("leaderboard")}>View Leaderboard</button>
              </div>
                          ...
                          <div key={person.id} style={styles.card}>
                            ...
                            <div style={{ marginTop: 8 }}>
                              ...
                              <button
                                ...
                                style={canMoveUp ? styles.smallBtn : { ...styles.smallBtn, opacity: 0.5, cursor: "not-allowed" }}
                                type="button"
                              >
                                ↑ Up
                              </button>
                              <button
                                ...
                                style={canMoveDown ? styles.smallBtn : { ...styles.smallBtn, opacity: 0.5, cursor: "not-allowed" }}
                                type="button"
                              >
                                ↓ Down
                              </button>
                ...
                {adminTab === "documents" && (
                  <>
                    <div style={styles.panel}>
                      ...
                      {(editingDocumentId || documentTitle || documentDescription || documentLink || documentButtonText || documentFile) && (
